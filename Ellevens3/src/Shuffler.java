...
public class Shuffler {
	...
	public static void perfectShuffle(int[] values) {
		...
		int[] shuffledValues = new int[VALUE_COUNT];
		int halfway = (VALUE_COUNT) /2;
		int k = 0;
		for (int j = 0; j < halfway; j++){
			shuffledValues[k] = values[j];
			k += 2;
		}
		k = 1;
		for (int j = halfway; j < VALUE_COUNT; j++ ){
			shuffledValues[k] = values[j];
			k += 2;	
		}
		for (int j = 0; j <VALUE_COUNT; j++){
			
			values[j] = shuffledValues[j];	
		}
...
//			int temp = values[r];
//			values[r] = shuffledValues[k];
//			shuffledValues[k] = temp;
		}
		}
	}
